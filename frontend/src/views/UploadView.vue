...
<template>
  <div class="app-content">
    <section class="app-panel" aria-labelledby="upload-title">
      <div class="panel-heading"><div><span class="panel-kicker">UPLOAD / CREATOR</span><h2 id="upload-title">分享新片段</h2><p>仅支持 MP4，最大 500 MiB。</p></div></div>
      <form class="upload-form" novalidate @submit.prevent="submit">
        <div class="upload-fields">
          ...
          <div class="field-group" :class="{ 'is-invalid': errors.description }"><label for="upload-description">简介 <span>最多 2000 个字符</span></label><textarea id="upload-description" v-model="form.description" name="description" maxlength="2000"></textarea><p class="field-error">{{ errors.description }}</p></div>
          <div class="field-group" :class="{ 'is-invalid': errors.file }"><label for="upload-file">视频文件</label><label class="file-picker" for="upload-file"><input id="upload-file" name="file" type="file" accept="video/mp4,.mp4" @change="selectFile"><span>选择 MP4</span><strong>{{ form.file?.name || '尚未选择文件' }}</strong></label><p class="field-error">{{ errors.file }}</p></div>
