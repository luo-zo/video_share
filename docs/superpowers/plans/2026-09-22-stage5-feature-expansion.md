...
**文档版本：** 2.0 · 2026-09-22  
**状态：** 待实施；复选框表示待办，不表示功能已完成。  
**目标：** 将现有视频社区 MVP 扩展为可分享、可回访、可治理、可演示的视频平台，并形成可复现的测试、性能和故障恢复证据。  
**代码基线：** 本次检查 main 为 cff21bd；实施前重新确认仓库状态。项目根目录为 C:/Users/27339/Desktop/video_share。  
...
这部分是回归基线，不重复计为第五阶段新增成果。完整接口入口见 [router.go](/C:/Users/27339/Desktop/video_share/backend/internal/server/router.go)，已有媒体处理说明见 [backend/README.md](/C:/Users/27339/Desktop/video_share/backend/README.md)。
...
- [ ] 记录 main 提交、未提交变化、Node/Go/Docker/FFmpeg 版本，识别当前运行服务的代码目录。
- [ ] 运行既有前后端测试，记录实际通过/失败/跳过项，不把旧报告当本次结果。
- [ ] 为注册、登录、发现、上传、播放、点赞收藏、评论、关注和作者管理录制或记录现有回归路径。
- [ ] 在 decisions.md 记录 Vue/TypeScript、Redis 首期范围、会话、统计和治理口径。
- [ ] 固定依赖版本及开发/测试/部署端口；不把旧 .worktrees 目录当第五阶段主项目。
...
- [ ] 建立 Vue 3 + TypeScript + Vite + Router + Pinia、vue-tsc、Vitest/Vue Test Utils 和 Playwright。
- [ ] 复用并逐步迁移现有 API 校验逻辑和样式；先登录，再发现/详情，最后投稿/个人中心。
- [ ] HLS 实例、事件监听、请求取消、轮询计时器在组件卸载/换视频时清理。
- [ ] 建立内部路径导航和 returnTo 校验；视频支持复制页面链接，浏览器返回保留搜索/分页状态。
- [ ] Vite 代理 /api，迁移旧 server.mjs 需要的安全/媒体行为；旧服务停止承担主入口后再删除无用实现。
- [ ] 纯函数旧 node:test 在迁移期可保留，逐项迁移而不以删除测试换取通过；最终 npm test 覆盖所有保留测试。
- [ ] 保留小黑猫、键盘导航、焦点、窄屏和 reduced-motion；Vue v-html 不渲染用户输入。
...
- [ ] 增加 Redis 服务、健康检查、go-redis 客户端、连接/命令超时和关闭处理；凭证从环境读取。
- [ ] 实现第 4.3 节限流，Lua 原子执行，所有状态键过期。
- [ ] 用两个 API 实例连接同一 Redis，验证合并后的配额，而非只测单进程。
- [ ] 验证 429/Retry-After、不同用户隔离、未配置代理头不能伪造 IP。
- [ ] 关闭 Redis，分别验证普通查询、身份、登录限流、评论/举报的预定行为。
- [ ] 增加缓存/限流超时和降级计数；Redis 故障不对所有 API 统一报同一个可用性状态。
...
- [ ] 实现 family/token 数据结构、事务轮换、sid 校验、退出/改密撤销。
- [ ] 前后端落实第 4.2 节冲突窗口、single-flight、跨标签协调、CSRF 和可信代理。
- [ ] 资料编辑昵称/简介；改密沿用 bcrypt 输入限制，不擅自改变既有密码规范。
- [ ] 测试两个并发刷新、窗口内冲突、窗口外旧 token 重放、过期、退出竞态、改密后旧 JWT 和 Cookie。
- [ ] 测试浏览器刷新恢复、网络暂时失败不登出、重试次数上限和重放写请求规则。
...
- [ ] 提供公开资料、公开视频、粉丝/关注分页及计数；计数过滤禁用账号。
- [ ] 视频作者名称、评论作者和关注列表跳转公开主页。
- [ ] 主页支持关注/取关，操作完成同步当前相关界面状态。
- [ ] 测试自己/他人/游客、空作品、私密作品、禁用用户和不存在 ID。
...
- [ ] 预置未分类、生活、知识、科技、游戏、音乐等演示分区；分类启用状态和展示顺序落库。
- [ ] 标签去首尾空白、Unicode 规范化和大小写标准化，1–20 个字符、最多 5 个，事务去重；测试中文、重音字符和并发新建。
- [ ] 投稿与编辑支持分区、标签、visibility；旧请求不带新增字段仍成功。
- [ ] 发现页支持筛选；关注流仅展示当前关注的正常作者公开作品。
- [ ] 相关推荐按共同标签数、同分区优先、旧累计播放/点赞、发布时间、ID 排序，明确为规则推荐。
- [ ] 分区实现第 4.3 节缓存，测试命中、失效、冷启动并发和 Redis 故障回源。
...
- [ ] 按第 4.4 节实现两层展示、独立分页、删除占位和同视频约束。
- [ ] 为 POST 幂等请求建立 operation_receipts，保存请求摘要和资源 ID；冲突请求体返回 409。
- [ ] 评论/关注、通知、必要计数同事务提交；定义固定锁顺序，死锁只对有幂等键的操作有限重试。
- [ ] 通知接收人去重、自通知排除、所有权校验、未读/全部已读、目标失效提示。
- [ ] 前台轮询、隐藏暂停；logout 时清空通知并忽略迟到结果。
...
- [ ] 普通用户可举报并查看自己的处理进度；管理员工作台接单、查看目标、填写原因、驳回或带处置结案。
- [ ] 提供显式 CLI 命令给已有用户授予/撤回管理员角色；无默认管理员密码、无公开自助提权路由。
- [ ] 执行隐藏/恢复、禁用/恢复、通知、计数调整和审计事务；禁止恢复已删内容。
- [ ] 对所有公开查询及 HLS/封面入口接入统一可见性条件，补缓存命中下的权限测试。
- [ ] 测试越权、自禁用、最后管理员保护、重复处置、两管理员同时接单和失败回滚。
- [ ] 演示关闭内容后新媒体请求被阻止，测量已签名对象访问的残余有效窗口。
...
- [ ] 完整落实第 4.5 节 session/seq/时钟上限；采用可注入时钟测试，不依赖测试真实等待。
- [ ] 点赞/收藏/评论/粉丝发生真实关系变化时写同事务净增量；上线前旧状态继续可取消且趋势允许负数。
- [ ] 汇总 7/30 天当前总数、日趋势、Top 10；不同指标日期和旧/新口径在 API 元数据及页面说明。
- [ ] 断点在 loadedmetadata 后恢复一次；短视频、临近结束、切视频、退出和请求乱序有用例。
- [ ] 测试自然播放、拖到结尾、暂停/缓冲、隐藏标签、多倍速、多标签、重放心跳、session 过期、跨午夜和零分母。
- [ ] 首版用表格、进度条与轻量图形展示；无需增加大型 BI 服务。
...
- [ ] 从 MySQL 日指标计算分数，ZSET 批量写临时键，原子替换；用明确时区生成日期。
- [ ] 采用单独、可重复执行的重建进程/命令，60 秒周期；多进程时使用带所有者校验与超时的构建锁或明确单实例约束。
- [ ] 相同分数使用固定次级顺序；分页只承诺同一生成快照下稳定，重新生成时间随响应返回。
- [ ] 命中榜单后仍从数据库过滤候选，补足到请求数量或确实无更多内容；结果空时展示合规空状态。
- [ ] 缓存丢失、Redis 重启、过期、重建中崩溃分别测试，旧完整榜或 MySQL 回源可用。
- [ ] 压测榜单命中/未命中，记录 SQL 次数及可见性过滤成本。
...
- [ ] Vue 构建为 dist，Nginx 提供静态/深链/API 代理；媒体域名、CORS、Cookie、CSP、请求限制逐项演示验证。
- [ ] Compose 编排 MySQL、Redis、MinIO、Kafka、API、Worker、前端服务，分开开发测试与演示配置。
- [ ] 迁移作为显式步骤单独执行；备份恢复演练只操作一次性测试数据，验证账号和视频元数据可恢复。
- [ ] CI 分前端类型/单元/构建、Go 单元/vet/build、真实依赖集成、浏览器 E2E；Linux runner 可用条件下增加 Go race 检查。
- [ ] 配置测试所需 MySQL/Redis/Kafka/MinIO 与 FFmpeg；环境缺失明确失败，不把跳过当验收通过。
- [ ] 文档提供环境变量名称、示例非秘密值、启动停止命令、媒体公共地址、管理员引导和故障定位。
- [ ] 在线演示发布作为交付后的独立操作；可先完成本地同等部署验收，不要求本任务购买服务器。
...
- [ ] 创建隔离作者/观众/管理员测试身份，完成注册—登录—刷新—资料—投稿—转码—发现—分享—互动—通知—举报—处置—恢复—续播—退出。
- [ ] 保留既有转码/社区脚本并完成兼容回归；新增 PowerShell 脚本在 Windows PowerShell 5.1 实际解析执行验证，使用 ASCII 或 UTF-8 BOM。
- [ ] 演练 Kafka 暂停再恢复，观察 Outbox 待发记录、重试与发布结果。
- [ ] 演练消息重复、Worker 中途退出与租约/任务恢复，确认不会重复发布或产生失控重试。
- [ ] 演练 Redis 不可用、热点过期、榜单重建、数据库事务失败和两管理员并发处置。
- [ ] 记录每次演练的步骤、期望、实际、日志、修复和复测；发现基线缺陷时建立修复记录，不修改期望掩盖失败。
- [ ] 只清理本次脚本标识的测试数据，不对主库执行全库清空。
...
**依赖：** T11。  
**文件：** perf/k6/categories.js、ranking.js、engagement.js；docs/stage5/performance.md、demo.md、resume-evidence.md、decisions.md。

- [ ] 固定测试环境：CPU/内存、镜像版本、数据规模、网络、并发模式、预热、持续时间、代码提交、随机种子和错误定义。
- [ ] 先 smoke，再基准负载，再递增压力；至少重复 3 次。使用相同数据与负载对比缓存关闭、冷缓存、热缓存。
- [ ] 测量吞吐、P50/P95/P99、错误率、Redis 命中率、SQL 次数、CPU/内存；同时验证内容正确性和权限。
- [ ] API 压测与视频分片吞吐分开；不能用分区缓存接口的 QPS 宣称整个平台或转码处理能力。
- [ ] 转码单独记录固定样片时长/分辨率、排队耗时、处理耗时和 Worker 并行配置。
- [ ] 性能指标先建立基线，再根据演示环境确定目标；不预写“支持十万用户”“提升 90%”等结果。
- [ ] 准备 5–8 分钟演示：小黑猫登录、投稿转码、主页分发、互动通知、治理、Redis 故障回退与一次性能对比。
- [ ] 简历材料映射“实际实现 -> 测试或报告 -> 设计取舍”，仅填写本人能解释的代码和真实结果。

**验收：** 他人可按脚本复现主要结果；报告包含失败及环境限制；尚未实现或未测量的能力不写成既成成果。
...
- [ ] Vue 既有功能等价迁移与新增页面。
- [ ] Redis 真实用途、限流配额、缓存策略、榜单重建。
- [ ] 五组版本化迁移、旧数据兼容与数据恢复记录。
- [ ] API 合同、会话/指标/治理状态规则与权限矩阵。
- [ ] CI、单元、真实依赖集成、浏览器 E2E、故障演练。
- [ ] Compose/Nginx 环境、README、管理员准备与演示步骤。
- [ ] 压测脚本、原始结果、性能分析和局限。
- [ ] 架构图、转码/刷新/互动时序图、设计决策和简历证据。
- [ ] 所有重大缺陷已修复复测；剩余限制明确列出。
- [ ] 第 1.2 节 F01–F12 逐条对照验收，不能用“页面存在”代替业务通过。
